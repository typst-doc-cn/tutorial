...
#ref-table((
  ...
  #calc.pow(4, 3)
  ````),
  ([函数调用传递内容参数], refs.writing-markup.with(reference: <grammar-content-param>), ````typ
  #emph[emphasis]
  ````),
  ([成员访问], refs.content-scope-style.with(reference: <grammar-member-exp>), ````typ
  #`OvO`.text
  ````),
  ([方法调用], refs.content-scope-style.with(reference: <grammar-method-exp>), ````typ
  #"Hello World".split(" ")
  ````),
