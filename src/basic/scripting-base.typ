...
为了知道第二段程序具体做了什么事情，我们需要首先了解Typst中已有的基本值类型。

如果你学过Python等语言，那么这将对你来说不是问题。在Typst中，常用的基本值类型并不多，它们是：

=== 「整数类型的字面量」（Integer Literals）

一个整数类型的字面量常见由一连串数字，以及一个可选的负号组成。相信你一定知道整数的含义。

#code(```typ
三个值 #(-1)、#0 和 #1 偷偷混入了我们内容之中。
```)

值得注意的是：有的时候Typst不支持在「井号」后直接跟一个值。这个时候无论值有多么复杂，都可以将值用一对圆括号包裹起来，从而允许Typst轻松解析该值。例如，Typst无法处理「井号」后直接跟随一个「连字符」（`-`）的情况：
...
=== 「布尔类型的字面量」（Boolean Literals）

一个布尔类型的字面量要么为`false`要么为`true`，它表示逻辑的确否。

#code(```typ
两个值 #false 和 #true 偷偷混入了我们内容之中。
```)

一般来说，我们不直接使用布尔类型。相反，当代码做逻辑判断的时候，会产生布尔值。
...
同样我们看见输出的文档为空，这是因为「变量声明」并不产生任何内容。
