...
有的时候，我们像想要访问字面量、变量与函数中存储的“信息”。例如，给定一个字符串`Hello World`，我们想要截取其中的第二个单词。但是根据我们有限的脚本知识，单词`World`就在那里，却没有什么好的方法得到它。这是因为字符串本身是一个整体，虽然它有信息，我们却缺乏了*访问*信息的方法。
...
我们知道`Hello world!!!`是一个代码块内容，而`text`英文单词的意思是“文本”。那么“代码块”的“文本”就是它的一个固有属性，或者说文本是代码块的「成员」。于是，我们通过「点号」获得代码块的文本信息。

而「方法」则是一种特殊的「成员」。准确来说，如果一个「成员」是一个对象的函数，那么它就被称为该对象的「方法」。

我们来看Hello World程序的第三行、第四行与第五行脚本：
...
它们输出了相同的内容，事实上它们是同一「函数调用」的不同写法。
...
我们之前已经学过，这正是「函数调用」的语法。我们还发现`str.split`的输出与`str-split`的输出一样，那么我们可以推断：`str.split(...)`实际上就是在做「函数调用」，只不过在语法上更为紧凑。

第五行则更进一步，简化了脚本语法。Typst约定`str.split(x, y)`可以简写为`x.split(y)`，如果：
+ `x`是`str`类型。
+ `x`是`str.split`的第一个参数。

我们将`x.split(y)`称为「方法调用」，即一种特殊的「函数调用」。这个规则可以进一步推及其他类型。
...
其二，方便现代IDE补全脚本。如果你使用了带脚本自动补全的编辑器，当你在`"Hello World"`后敲下一个「点号」时，IDE会自动联想你接下来可能需要与“字符串”相关的函数，你便可以通过`.split`很快定位到“字符串拆分”这个函数。与之相对，如果使用`str-split`或`str.split`，我们首先需要敲下一个`s`字符，很快编辑器就会把所有以`s`开头的函数都告诉你了，这可就有点头疼了。

其三，方便管理相似功能的函数。你很快就可以联想到，不仅仅是字符串可以拆分，似乎内容也可以拆分。不仅如此，我们将来可能还有各种各样的拆分。如果需要为他们都取不同的名字，那可就太头疼了。相比`str.split`就简单多了。要知道，程序员最讨厌给变量和函数想命名这种东西了。

接下来我们只需要理解最后一行脚本，我们就彻底搞懂这个Hello World程序了。看起来，它也是属于其他什么类型的一个「方法」。
...
我们先看数组字面量。在Typst中，数组是按照顺序存储的y一些「值」，你可以在数组中存放任意内容。你可以使用圆括号与一个逗号分隔的列表创建一个数组：
...
含义不言而喻：使用空格拆分字符串我们可以得到两个单词，使用`at`方法，当「索引」参数为1的时候，我们就取出了其中的第二个元素，即`"Hello World"`程序中的第二个单词。

你可以使用圆括号与一个逗号分隔的列表创建一个字典字面量。但是与数组略微不同的是，列表的每一项是由冒号分隔的「键值对」。
...
其中“neco-mimi”、“utterance”和“attribute”是字典的「键」，它们必须是字符串。而`2`、`"喵喵喵"`和`[kawaii\~]`分别是对应的「值」。所谓「字典」即是「键值对」的集合。

为了访问字典，你可以使用`at`方法。只不过这次，由于「键」都是字符串，与数组使用整数作为「索引」相对应，你需要使用字符串作为字典的「索引」。
...
当`if`表达式没写`else`代码块而条件为假时，结果为`none`。

“none”在中文里意思是“无”，“什么都没有”。在程序执行中，有很多地方都会产生`none`，作为教程我们不会将它们一一列出。但我们可以指出`none`在「可折叠」的值（见上一节）中很重要的一个性质：`none`在折叠过程中被忽略。

见下程序用于根据数组所包含的值输出特定字符串：
...
基于以下迭代，我们检查`break`和`continue`语句的作用。
