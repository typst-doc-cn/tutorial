...
Typst很快，并不是因为它的#term("parser")和#term("interpreter")具有惊世的性能。经简单测试，与LaTeX作对比，单线程LaTeX宏引擎的*计算效率*甚至要优于Typst解释器。然而，这种计算的速度优势在Typst的良好架构下立即消弭了。由于较好的缓存策略，光是在Text Shaping方面，Typst单次编译就已然快于LaTeX。架构上，Typst是较优的；随着时间积累，Typst还可以积累和打磨更多程序优化。

由于缓存优势，Typst不仅单次编译很快，增量编译的速度更是达到了惊人的毫秒级，甚至微秒级。自然，这也影响到了Typst的语言特性，影响到我们将学到的脚本特性。

// 纵览Typst的编译流程，其大致分为4个阶段，解析、求值、排版和导出，层层有缓存。
...
自本节起，本教程将进入第二阶段，希望不仅让你了解如何使用脚本排版文档，还顺带讲解Typst的执行原理。当然，我们不会陷入Typst的细节。所述原理皆是希望能让你更能掌握脚本各种特性从何而来。
...
脚本从一加一开始学起。这一节，我们将完整描述Typst所有的基本字面量。上一节我们其实已经见过了一些字面量，但皆凭直觉使用：```typc 1```不就是数字吗，那么在Typst中也便如此。
...
字符串中的转义序列与「标记模式」中的转义序列语法相同，但内容不同。Typst只允许字符串中存在如下转义序列：
...
「函数声明」一共分为5个部分，相较于「变量声明」，多了一个参数列表。
...
#term("parameter identifier")的规则与功能与#term("variable identifier")相似，可用于后续使用该参数时引用。
...
我们可以看到其以箭头为体，两侧为参数列表和函数体，且不像「函数声明」需要标识符。

因此，「闭包」很适合用来作为参数传入其他函数，即作为「回调」函数。我们将会在下一章经常使用「回调」函数。例如，Typst提供一个`locate`函数。其接受一个函数参数。你可以使用「函数声明」：

#code(```typ
#let f(loc) = [当前页面为#loc.page()。]
#locate(f)
```)
