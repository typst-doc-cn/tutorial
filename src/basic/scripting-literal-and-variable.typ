...
在Typst中，用户定义的函数被称为#term("closure")，这与函数式编程中的概念类似。Typst因性能需求，需要保证函数的纯粹性，因此闭包中捕获的是变量的值（capture by value）。在Typst中我们不再区分「函数」与「闭包」。

#pro-tip([
  这与其他语言中的Lambda表达式、匿名函数等概念类似，甚至使用了大家熟悉的 `=>` 符号。如果你熟悉这些概念，你完全可以将Typst的闭包理解为与它们相同的概念。
])

按照如下方式我们可以定义一个闭包并将其赋值给变量 `f`：

#code(```typ
#repr((x, y) => [两个值#(x)和#(y)偷偷混入了我们内容之中。]) \
#((x, y) => [两个值#(x)和#(y)偷偷混入了我们内容之中。])([小明], [小红]) \
#let f = ((x, y) => [两个值#(x)和#(y)偷偷混入了我们内容之中。])
```)

Typst也提供了语法糖，可以用简写来定义闭包并赋值，使其与其他语言中的形式更接近：

#code(```typ
#let f(x, y) = [两个值#(x)和#(y)偷偷混入了我们内容之中。]
```)

我们看见输出的文档为空，与其他变量声明相同，函数声明（即声明一个变量并赋值为闭包）并没有特殊之处
...
== 函数进阶 <grammar-closure>

闭包是一个很有用的语法。上一节我们学习了简单函数的「函数声明」。一个简单的「闭包」如下：
...
参数允许包含「具名参数」。「具名参数」需要指定默认值：

#pro-tip([
  使用具名参数的一大有点是函数的使用者不再需要记忆或查询函数的每个参数的位置与其语义的关联，而是可以用名称来指代。这也就是说，函数在调用时传入具名参数是与位置无关的。不指定默认值的参数则成为受传入顺序影响的#term("positional arguments")。
])
...
#pro-tip([
  ...
  与Python等语言不同，Typst中不具名的参数是不能通过指定参数名的方式来传入的，例如下列代码不能正常编译：

  ```typ
  #let f(x, y: none) = [你好呀 #x, #y]
  #f(y: [OwO], x: 324)
  ```
...
== 副作用 <grammar-closure-context>

Typst中的闭包都是纯函数，而纯函数是没有副作用的，这在性能上带来了很大的优势，但在设计时又会带来不便。在Typst的v0.11.0版本中提供了一种折中方案：context表达式

todo：context是否应该单独放一节？

以下是一个“不那么纯”的函数

#code(```typ
#let x = state("x", 1)
#let f() = context [x = #x.get() \ ]
#f()
#x.update(2)
#f()
```)

可以看到我们结合了context表达式和state来实现一个有副作用的闭包。

todo: 补上link

#pro-tip([
  实际上，context表达式是在文档编译的后一阶段计算的，见 ... \
  而这与其他语言（如JavaScript中的Promise）有一定的相似之处，若你熟悉这些语言，考虑相关概念对于理解context表达式会十分有帮助。
])
