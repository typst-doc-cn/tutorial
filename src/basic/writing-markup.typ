...
#pro-tip[
  本书会同时携带一些“Pro Tip”，由蓝色框包裹，告诉你一些更难理解或需要更多背景知识的知识点。
  
  你可以在初次阅读时跳过这些框，而不影响你对正文的理解。建议你在阅读完整本书后倒回来观看你当初不理解的知识点。
]
...
例如，#mark("*")作为定界符赋予所包裹的一段文本以#term("strong semantics")。

#code(```typ
着重语义：这里有一个*重点！*
```)

与#term("strong semantics")类似，#mark("_")作为定界符将赋予#term("emphasis semantics")：
...
例如，配对的#mark("`")包裹一段内容，表示内容为#term("code block")。

#code(````typ
短代码片段：`code`
````)

有时候你希望允许代码内容包含#mark("`")。这时候，你需要使用*至少连续*三个#mark("`")组成定界符标记#term("code block")：
...
除了定界符的长短，代码片段还有是否成块的区分。如果代码片段符合以下两点，那么它就是一个#term("blocky code block")：
+ 使用*至少连续*三个#mark("`")作为长定界符。
+ 包含至少一个#term("line break")。
...
在Typst中，你可以使用一个的#mark("-")开启一个无编号列表项，并使用缩进调整列表项等级：
...
你有时不希望使用Typst提供的语义标记，而是直接展示标记符号本身的真实内容。例如，你可能希望在段落中直接展示一个#mark("_")，而非期望使用#mark("_")赋予强调语义。这时你需要使用#term("escape sequences")：
...
如此诸多#term("escape sequences")无需死记硬背，你只需要记住，如果你希望录入一个字符，但是：
+ 其在Typst中已经被赋予含义，请尝试在字符前添加一个#mark("\\")。
...
空格的#term("shorthand")是#mark("~")：

#code(```typ
AB v.s. A~B
```)

连接符的#term("shorthand")由两个连续的#mark("-")组成：

#code(```typ
北京--上海路线的列车正在到站。
```)

破折号的#term("shorthand")由三个连续的#mark("-")组成（有问题，毋用，请直接使用em dash，`—`）：
