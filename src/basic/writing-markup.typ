...
Typst希望用户总是尽可能少的配置样式，就获得一个排版优秀的文档。多数情况下，你只需要专心撰写文档，而不需要在文档内部对排版做任何更复杂的调整。

得益于此，为了使你可以编写一篇基本文档，本节仍只需涉及最基本的语法。哪怕只依靠这些语法，你已经可以编写满足很多场合需求的文档。

#pro-tip[
  本书会同时携带一些“Pro Tip”，由蓝色框包裹。它们告诉你一些较难理解或需要更多背景知识的知识点。
  
  你可以在初次阅读时*跳过*这些框，而不影响对正文的理解。但建议你在阅读完整本书后倒回来观看当初不理解的那些“Pro Tip”。
]
...
与许多标记语言相同，Typst中使用一系列#term("delimiter")规则确定一段语义的开始和结束。为了组成一段带有语义的文本，需要将一个#term("delimiter")置于文本之前，表示该语义的开始；同时将另一个#term("delimiter")置于文本末尾，表示该语义的结束。
...
Typst的#term("code block")标记语法与Markdown完全相同。如果你使用过Markdown，那么你将对以下#term("code block")的标记方式非常熟悉。

例如，配对的#mark("`")包裹一段内容，表示内容为#term("code block", postfix: "。")

#code(````typ
短代码片段：`code`
````)

有时候你希望允许代码内容包含#mark("`")。这时候，你需要使用*至少连续*三个#mark("`")组成定界符标记#term("code block", postfix: "：")
...
除了定界符的长短，代码片段还有是否成块的区分。如果代码片段符合以下两点，那么它就是一个#term("blocky code block", postfix: "：")
+ 使用*至少连续*三个#mark("`")作为长定界符。
+ 包含至少一个#term("line break", postfix: "。")
...
在Typst中，你可以使用一个#mark("-")开启一个无编号列表项，并使用缩进调整列表项等级：
...
与之相对，#mark("+")开启一个有编号列表项，并可以与无编号列表项相混合。
...
你有时不希望使用Typst提供的语义标记，而是直接展示标记符号本身的真实内容。例如，你可能希望在段落中直接展示一个#mark("_", postfix: "，")而非期望使用#mark("_")赋予强调语义。这时你需要使用#term("escape sequences", postfix: "：")

#code(````typ
在段落中直接使用下划线 >\_<！
````)

遵从了大部分语言的习惯，Typst使用#mark("\\")转义特殊标记。下表给出了所有需要转义的字符：
...
以上大部分#term("escape sequences")都紧跟一个单字符，除了表中的最后一列。

`\u{unicode}`的语法常见于各种语言。你可以通过将`unicode`替换为#link("https://zh.wikipedia.org/zh-cn/%E7%A0%81%E4%BD%8D")[Unicode码位]的值，以输出该特定字符，而无需*输入法支持*。例如，你可以这样输出一句话：

#code(````typ
\u{9999}\u{8FA3}\u{725B}\u{8089}\u{7C89}\u{597D}\u{5403}\u{2665}
````)

如此诸多#term("escape sequences")无需死记硬背，你只需要记住：
...
空格的#term("shorthand")是#mark("~", postfix: "：")

#code(```typ
AB v.s. A~B
```)

连接符的#term("shorthand")由两个连续的#mark("hyphen")组成：
...
采用C语言风格的注释语法，Typst的#term("comment")有两种表示方法。
