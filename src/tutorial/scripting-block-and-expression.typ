...
根据大的分类，可用的表达式可以分为6类（它们并非严格术语，而是为了方便教学而分类），它们是：
+ 代数运算表达式。
+ 逻辑比较表达式。
+ 逻辑运算表达式。
+ 赋值表达式。
...
其中，「项」像是表达式的原子。字面量、变量、函数调用、类型转换和代码块都可以作为项，然后继续参与更复杂的表达式。

== 代数运算表达式 <grammar-arith-exp>

Typst支持对数字的算术运算，其中浮点运算遵守IEEE-754标准。整数和浮点数之间可以混合运算：
...
从上可以看出，整数和整数运算尽可能*保持*整数，但是整数和浮点数运算则结果变为浮点数。除法运算的结果是浮点数。算术之间遵守四则运算规则。

单独的正号和负号也是算术运算表达式。正号表示取正<grammar-plus-exp>，不会改变数字本身；负号表示取负<grammar-minus-exp>，会改变数字符号：
...
除了数字运算，字符串、数组等还支持加法和乘法运算。它们的加法实际上是连接操作<grammar-string-concat-exp>，它们的乘法则是重复元素的累加<grammar-string-mul-exp>。
...
字典只支持加法操作。若有重复键值对，则右侧字典的键值对会覆盖左侧字典的键值对。
...
+ 整数转换陷阱。为了转换类型，可以使用`int`，但有可能产生精度损失：
  ...
  当前Typst会将浮点数向零取整为整数，而不是四舍五入。若你需要向下取整或向上取整，应显式使用`calc.floor`或`calc.ceil`。
...
字符串、数组和字典之间也是可以比较的，理解它们则必须从偏序关系和等价关系入手。
...
对于单个字符，我们依照字典序比较<grammar-string-cmp-exp>，其中容易理解地是数字字符顺序按字面递增。对于前两者，我们可以看到`"1"`比`"2"`小；进而，故"113"比"121"小；尽管如此，优先判断长度关系，故"2"比"12"小。
...
类型转换并不是新的运算符。类型本身可以作为函数调用，因此类型转换本质上仍是函数调用表达式。由于它常常和字面量、运算结果同时出现，这里也一并列出几种常用写法。

整数可以转换成浮点数：<grammar-int-to-float>

#code(```typ
#float(1), #(type(float(1)))
```)

布尔值可以转换成整数：<grammar-bool-to-int>
...
浮点数可以转换成整数：<grammar-float-to-int>

#code(```typ
#int(1.9), #int(-1.9), #(type(int(1.9)))
```)

这个转换会向零取整。也就是说，`1.9`转换为`1`，`-1.9`转换为`-1`。

十进制字符串可以转换成整数：<grammar-dec-str-to-int>

#code(```typ
#int("1"), #(type(int("1")))
```)

如果希望把十六进制、八进制或二进制的代码字符串转换成整数，可以先对字符串求值，再检查求值结果确实是整数：<grammar-nadec-str-to-int>
...
布尔值没有直接的字符串构造器，但可以用`repr`获得它的代码表示：<grammar-bool-to-str>

#code(```typ
#repr(false), #(type(repr(false)))
```)

Typst也没有把数字直接转为布尔值的构造器。日常代码中，若你需要类似的规则，可以用比较表达式自己定义：<grammar-int-to-bool>
...
「代码块」和值（表达式）的关系与「控制流」息息相关。我们将会在下一节#(book.cross-link.with("/tutorial/scripting-control-flow.typ"))[《控制流》]详细介绍。
