...
自本节起，本教程将进入第二阶段，希望不仅让你了解如何使用脚本，还一并讲解Typst的执行原理。当然，我们不会陷入Typst的细节。教程中出现的原理都是为了帮助你更好地理解语言本身。
...
#pro-tip[
  #term("introspection function")是指那些用于获取解释器内部状态的函数。它们往往接受一些语言对象，并返回存储在解释器内部的相关信息。在这里，#typst-func("repr")接受任意值，并返回对应的代码表示。
]
...
我们将介绍所有基本字面量，这是脚本的“一加一”。其实在上一节，我们已经见过一部分字面量，但皆凭直觉使用：```typc 1```不就是数字吗，那么在Typst中，它就是数字。与之相对，TeX底层没有数字和字符串的概念。

如果你学过Python等语言，那么这对你来说不是问题。在Typst中，常用的字面量并不多，它们是：
+ #term("none literal")。
...
一个布尔字面量表示逻辑的确否。它要么为```typc false```（假）<grammar-false-literal>，要么为```typc true```（真）<grammar-true-literal>。
...
浮点数与整数非常类似。最常见的浮点数由整数部分或小数部分组成：
...
类型本身也可以当作函数使用，例如`int`类型可以接受字符串，转换成整数。

#code(```typ
#int("11"), #int("-23")
```)

从转换的角度，`eval`可以将代码字符串转换成值。例如，你可以转换十六进制数字：
...
// + Typst支持增量解析文件。
// + Typst所有由*用户声明*的函数都是纯的，在其上的调用都是纯的。例如Typst天生支持快速计算递归实现的fibonacci函数：
...
+ 查看#(refs.ref-wasm-plugin)[《参考：WASM插件》]，以掌握在Typst中使用Rust、JavaScript、Python等语言编写插件库的方法。
