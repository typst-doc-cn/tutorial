...
Typst希望你总是尽可能少配置样式，也能获得一个排版精良的文档。多数情况下，你只需要专心撰写文档，而不需要在文档内部对排版做任何更复杂的调整。

得益于此设计目标，如果只是为了让你可以用Typst编写一篇基本文档，本节仍只需涉及最基本的语法。哪怕只依靠这些语法，你也已经可以编写满足很多场合需求的文档。
...
与许多标记语言相同，Typst中使用一系列#term("delimiter")规则确定一段语义的开始和结束。若要赋予语义，需要将一个#term("delimiter")置于文本*之前*，表示某语义的开始；同时将另一个#term("delimiter")置于文本*之后*，表示该语义的结束。
...
有时候你希望允许代码内容包含换行或#mark("`")。这时候，你需要使用*至少连续*三个#mark("`")作为#term("raw block")的定界符：<grammar-long-raw>
...
有编号列表项可以与无编号列表项混合。<grammar-mix-list-emum>
...
以上大部分#term("escape sequences")都只需在反斜杠后紧跟单个字符；表中的最后一列除外。 <grammar-unicode-escape-sequences>

表中的最后一列所展示的`\u{unicode}`语法被称为Unicode转义序列，也常见于各种语言。你可以将`unicode`替换为#link("https://zh.wikipedia.org/zh-cn/%E7%A0%81%E4%BD%8D")[Unicode码位]的值，输出该特定字符，而无需*输入法支持*。例如，你可以这样输出一句话：
...
输出换行符是一种特殊的#term("escape sequences")，它使文档输出换行。

#mark("\\")后紧接任意一个#term("whitespace")，表示在此处主动插入一个段落内的换行符： <grammar-newline-by-space>
...
完整的速记符号列表可参考#link("https://typst.app/docs/reference/symbols/")[Typst Symbols]。
...
与代码片段的情形类似，Typst也提供了另一种可以跨行的#term("comment")，形如`/*...*/`。<grammar-cross-line-comment>
...
基于《初识标记模式》掌握的知识，你应该可以编写一些非常简单的文档。
...
#exercise[
  使源码至少有两行，第一行包含“欲穷千里目，”，第二行包含“更上一层楼。”，但输出时不换行也不空格：#rect(width: 100%, eval(q1.text, mode: "markup"))
][
  #q1
]
