...
+ #typst-func("strong")是赋予#term("strong semantics")的函数。
...
#term("content block")不会影响包裹的内容——Typst仅仅是把内部代码解析为#term("content block")的内容。#term("content block")也*几乎不影响内容的书写*。
...
在接下来两小节你将看到Typst作为一门*编程语言*的核心设计，也是进行更高级排版必须要掌握的知识点。由于我们的目标首先仅是*编写一篇基本文档*，我们将尽可能少引入新的知识点，只介绍其中最简单常用的语法。

== 解释模式 <grammar-enter-script>

```typ #strong[]```语法第一点提到：`#`使解释器进入#term("code mode")。
...
这涉及到Typst的编译原理。Typst是一个动态解释器，其按顺序查看并#term("interpret")你的文档源码。而#mark("#")就是告诉当前处于「标记模式」的解释器，接下来请转为「脚本模式」。

实际上，相比Python等语言，Typst的确有些特殊。它的解释器可以在不同#term("interpreting mode")下运行。在不同模式下，解释器以不同的语法规则解释你的文档。这种设计借鉴了LaTeX的文本和数学模式。Typst一共有三种#term("interpreting mode")：标记模式的语法更适合你组织文本，脚本模式更适合你书写脚本，而数学模式则最适合输入复杂的公式。

// todo 三种解释模式的visualization

=== 标记模式

当解释器解释一个文件时，其默认就处于#term("markup mode")。在这个模式下，你可以使用各种记号创建标题、列表、段落……Typst语法几乎就和Markdown一样。这些标记，我们大多在之前的章节已经学过。
...
当处于「脚本模式」时，解释器在*适当*的时候从「脚本模式」退回为「标记模式」。如下所示，在「脚本模式」下解析到数字```typc 2```后，解释器回到了「标记模式」：
...
Typst解释器一共有三种模式，其中两种我们之前已经介绍。剩下的最后一种被称为#term("math mode")。很多人认为Typst针对LaTeX的核心竞争点之一就是优美的#term("math mode")。
...
由于使用#term("math mode")有很多值得注意的地方，且#term("math mode")是一个较为独立的模式，本书将其单列为一章参考，可选阅读。如果需要在文档中插入数学公式，请移步#(refs.ref-math-mode)[《参考：数学模式》]。
...
与大部分语言相同，在调用Typst函数时，你可以向其传递以逗号分隔的#term("value")，这些#term("value")被称为函数的参数。

#code(```typ
四的三次方为#calc.pow(4, 3)。
```)

这里#typst-func("calc.pow")是编译器内置的幂计算函数，它接受两个参数：
+ 一为```typc 4```，为幂的底
+ 一为```typc 3```，为幂的指数。
...
首先，中括号包裹的是一段内容。在之前已经学到，这是一个#term("content block")。然后，#term("content block")被放在参数列表中，说明它是#typst-func("strong")的参数。#typst-func("strong")与幂函数调用没有什么语法上的区别，无非是接受了一个#term("content block")作为参数。
...
在许多语言中，所有函数参数必须包裹在函数调用参数列表的「圆括号」之内。

#code(```typ
着重语义：这里有一个#strong([重点！])
```)

但在Typst中，如果将内容块作为参数，它可以紧贴在参数列表的「圆括号」之后。
...
这种传参方式被称为#(refs.scripting-base)[「具名传参」]。冒号的左侧写`fill`，表示令`fill`参数为冒号右边的`orange`（橘色）。
...
文本本身也可以设置一些「具名参数」。与#typst-func("strong")和#typst-func("emph")类似，文本也有一个对应的元素函数#typst-func("text")。#typst-func("text")接受任意内容，返回一个会影响内部文本的结果。
...
当输入是一段内容时，返回的是该内容本身，但是对于内容中的每一个文本元素，都会应用相应的文本属性修改。下例将「代码片段」元素中的文本元素修改为红色：
...
你还可以用逗号分隔的「列表」为文本同时设置多个字体。Typst优先使用列表中靠前的字体。例如，可以同时设置西文为Times New Roman字体，中文为仿宋字体：
...
`set`关键字后跟随一个函数调用语法，表示此后的对应元素都具有指定的样式。这比```typ #text(fill: red)[红色鸭梨]```要更易读。

默认情况下文本元素的`fill`参数为黑色，即文本默认为黑色。经过`set`规则，往后的文本都默认为红色。
...
#pro-tip[
  ...
  从这个角度，你很容易就知道鸭梨文本是蓝色的。
]

本节前面讲述的所有元素的「具名参数」都可以如是设置，例如文本大小、字体等。

关于「`set`」语法更详细的介绍，详见#(refs.content-scope-style)[《内容、作用域与样式》]。

== 图像 <grammar-image>

图像对应的元素函数是#typst-func("image")。

你可以通过#(refs.scripting-modules)[绝对路径或相对路径]加载图片文件：
...
#typst-func("figure")不仅可以接受#typst-func("image")作为内容，也可以接受任意内容：
...
#pro-tip[
  ...
  具体来说，标签会附加到它的#term("syntactic predecessor")。可以对比上例。
...
你可以通过#typst-func("table")函数创建表格。#typst-func("table")接受一系列内容，并根据参数将内容组装成一个表格。如下例，通过`columns`参数设置表格为2列，Typst自动为你生成一个2行2列的表格：
...
其他可选对齐方式有`left`、`right`、`bottom`、`top`、`horizon`等，详见#(refs.ref-layout)[《参考：布局函数》]。

== 使用其他人的模板

虽然这是一篇教你写基础文档的教程，但是为什么不更进一步？有赖于Typst将样式与内容分离，如果有朋友愿意为你分享两行神秘代码，当你将其粘贴到文档开头时，你的文档将会变得更为美观：
...
#exercise[
  用#typst-func("underline")实现“删除线”效果，其中删除线距baseline为`40%`：#rect(width: 100%, eval(q1.text, mode: "markup"))
][
  #q1
]
